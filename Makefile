...
.PHONY: help install dev lint check test docker-build docker-run setup-iam-dev setup-iam-prod deploy-dev deploy-prod health-dev health-prod chat-dev chat-prod chat-dev-debug
...
docker-run:
	docker run --rm -p 8080:8080 reno-agent
