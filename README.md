...
REGION=europe-west1
REPOSITORY=reno-agent
IMAGE_NAME=reno-agent
SERVICE_NAME=reno-agent-dev
TAG=dev-local
...
GitHub Actions uses the same variables from the `dev` and `prod` GitHub
environments. The `GCP_SA_KEY` environment secret must contain the JSON key for
...
Deployment builds and pushes the image to Artifact Registry through Cloud Build,
then deploys that exact image to Cloud Run.
